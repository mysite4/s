...
def add_appointment(request):
    if request.method == 'POST':
        ...
        if not all([name, phone, doctor_id, date, time]):
            return JsonResponse({'success': False, 'error': 'الحقول المطلوبة ناقصة'})

        # تحقق إذا المريض موجود مسبقًا
        ...
        # إنشاء الموعد
        appointment = Appointment.objects.create(
            name=name,             # إذا الحقل في الموديل هو name
            phone=phone,           # إذا الحقل موجود
            doctor_id=doctor_id,
            date=date,
            time=time,
            message=message,
            patient=patient        # إذا عندك حقل patient
        )
