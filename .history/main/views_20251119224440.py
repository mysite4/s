...
def accounting_dashboard(request):
    # جلب البيانات وتجميعها حسب الشهر
    data = Invoice.objects.values('date__month').annotate(total=Sum('amount')).order_by('date__month')

    invoice_labels = [f"Month {item['date__month']}" for item in data]
    income_data = [item['total'] for item in data]

    total_income = sum(income_data)  # حساب مجموع الإيرادات
